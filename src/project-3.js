...
const hasEmail = (user) => {
  ...
  return (('email' in user) ? user['email'].length > 0 : false)
};
...
const sumUserPostLikes = (user) => {
  ...
  // return the sum
  return reduce(user.posts, (acc, post) => { return acc + post.likes }, 0)
};
...
const addCalculateDiscountPriceMethod = (storeItem) => {
  ...
  storeItem.calculateDiscountPrice = () => { return storeItem.price * (1 - storeItem.discountPercentage) }
  return storeItem
};
